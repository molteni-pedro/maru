	...
	class Jet_Engine_Dynamic_Tags_Manager {
		...
		public function add_listing_item_dynamic_css( $content, $listing_id, $inner_templates ) {
			...
			foreach ( $post_ids_for_data as $post_id_for_data ) {
				...
				if ( ! $this->force_print_css && in_array( $handle, $this->printed_css ) ) {
					continue;
				}
		...
		/**
		 * Set the `force_print_css` property.
		 *
		 * @param boolean $force
		 */
		public function force_print_css( $force = false ) {
			$this->force_print_css = $force;
		}
